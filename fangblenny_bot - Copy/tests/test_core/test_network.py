...
class TestSafeRequest:
    ...
    @patch('fangblenny_bot.core.network.get_thread_session')
    def test_safe_request_skips_host_during_dns_backoff(self, mock_get_session):
        from fangblenny_bot.core import network

        with network._host_backoff_lock:
            network._host_backoff_until["example.com"] = time.time() + 5
        try:
            result = safe_request('GET', 'http://example.com/test')
            assert result is None
            mock_get_session.assert_not_called()
        finally:
            with network._host_backoff_lock:
                network._host_backoff_until.clear()

    @patch('fangblenny_bot.core.network.get_thread_session')
    def test_safe_request_records_dns_backoff(self, mock_get_session):
        from fangblenny_bot.core import network
        ...
        try:
            ...
            assert result is None
            with network._host_backoff_lock:
                assert network._host_backoff_until["example.com"] > time.time()
        finally:
            with network._host_backoff_lock:
                network._host_backoff_until.clear()
