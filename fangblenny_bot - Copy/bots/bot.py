...
Phemex Trading Bot
======================================
Runs on LIVE production market data.
Can operate in 'live' or 'simulation' (paper trading) mode.
"""
...
def set_mode_config(mode: str):
    """Sets the global file paths based on the bot's operating mode."""
    global BOT_MODE, ACCOUNT_FILE, COOLDOWN_FILE, LOGS_FILE, RESULTS_FILE, CONFIG_FILE, TRADES_FILE
    BOT_MODE = mode
    if mode == "sim":
        ACCOUNT_FILE   = DASHBOARD_PUBLIC_DIR / "paper_account.json"
        COOLDOWN_FILE  = DASHBOARD_PUBLIC_DIR / "sim_cooldowns.json"
        LOGS_FILE      = DASHBOARD_PUBLIC_DIR / "sim_logs.json"
        RESULTS_FILE   = DASHBOARD_PUBLIC_DIR / "sim_trade_results.json"
        CONFIG_FILE    = DASHBOARD_PUBLIC_DIR / "sim_config.json"
        TRADES_FILE    = SCRIPT_DIR / "bot_trades.json"
    else: # live
        ACCOUNT_FILE   = DASHBOARD_PUBLIC_DIR / "live_account.json"
        COOLDOWN_FILE  = DASHBOARD_PUBLIC_DIR / "live_cooldowns.json"
        LOGS_FILE      = DASHBOARD_PUBLIC_DIR / "live_logs.json"
        RESULTS_FILE   = DASHBOARD_PUBLIC_DIR / "live_trade_results.json"
        CONFIG_FILE    = DASHBOARD_PUBLIC_DIR / "live_config.json"
        TRADES_FILE    = SCRIPT_DIR / "live_bot_trades.json"
...
def tui_log(msg: str) -> None:
    ...
    with _log_lock:
        ...
        if len(_bot_logs) > _max_logs:
            _bot_logs.pop(0)
        try:
            LOGS_FILE.write_text(json.dumps(_bot_logs))
        except OSError:
            pass
...
def load_account() -> dict:
    """Loads the account file, creating it with defaults if it doesn't exist."""
    with ACCOUNT_LOCK:
        ...
        try:
            ...
            logger.error(f"Failed to decode {ACCOUNT_FILE.name} — reinitializing.")
            data = {"balance": INITIAL_BALANCE, "positions": []}
            ACCOUNT_FILE.write_text(json.dumps(data, indent=2))
            return data
    ...
    with _cooldown_lock:
        ...
        for s, data in LAST_EXIT_TIME.items():
            if isinstance(data, (tuple, list)) and len(data) == 2:
                ts, sc = data
                dur = bot_core.get_cooldown_duration(sc)
                if time.time() - ts < dur:
                    active_exit[s] = [ts, sc]
            else:
...
def _ws_run_loop() -> None:
    ...
    while True:
        ...
        time.sleep(2.0)
        if not load_account().get("positions"):
            break
...
def _subscribe_symbol(symbol: str) -> None:
    ...
    def _do_sub() -> None:
        ...
        if _ws_app and _ws_app.sock and _ws_app.sock.connected:
            symbols = [p["symbol"] for p in load_account().get("positions", [])]
...
def _log_closed_trade(
    ...
    try:
        with _file_io_lock:
            with open(RESULTS_FILE, "a", encoding="utf-8") as _fh:
        ...
        try:
            while True:
                ...
                if not _display_paused.is_set():
                    bot_core.make_entity_request("Snapshot", data={
                        "timestamp": datetime.datetime.now().isoformat(),
                        "equity": equity,
...
def verify_sim_candidate(symbol: str, direction: str, original_score: int, wait_seconds: int = 20) -> Optional[dict]:
    ...
    for i in range(steps):
        ...
        cfg = {
            "TIMEFRAME": bot_core.TIMEFRAME,
            "MIN_VOLUME": bot_core.MIN_VOLUME,
            "RATE_LIMIT_RPS": bot_core.RATE_LIMIT_RPS,
            "CANDLES": 100
        }
...
def execute_sim_setup(result: dict, direction: str) -> bool:
    ...
    with _stop_lock:
        ...
        if last_exit_data:
            ...
            cooldown_dur = bot_core.get_cooldown_duration(last_score)
            if time.time() - last_exit < cooldown_dur:
                remaining_m = (cooldown_dur - (time.time() - last_exit)) / 60
                tui_log(f"COOLDOWN: {symbol} — {remaining_m:.1f}m remaining before re-entry")
                return False
        ...
        current_leverage = bot_core.get_score_leverage(score)
        ...
        tp1_mult = bot_core.TAKE_PROFIT_PCT * 0.5
        tp2_mult = bot_core.TAKE_PROFIT_PCT * 0.75
        tp3_mult = bot_core.TAKE_PROFIT_PCT
        
        if direction == "LONG":
            stop_px  = price * (1.0 - bot_core.TRAIL_PCT)
    ...
    bot_core.make_entity_request("Trade", data={
        "trade_id": trade_id,
        "symbol": symbol,
        "direction": direction,
    ...
    bot_core.make_entity_request("MarketContext", data={
        "timestamp": datetime.datetime.now().isoformat(),
        "symbol": symbol,
...
def on_scan_result(r: dict, direction: str) -> None:
    # Respect BOT_DIRECTION from environment
    bot_dir = os.getenv("BOT_DIRECTION", "BOTH").upper()
    if bot_dir != "BOTH" and direction.upper() != bot_dir:
        return
...
def sim_bot_loop(args) -> None:
    ...
    with _display_lock:
        ...
        while True:
            # 1. Capture dynamic parameters at start of loop for consistency
            load_dotenv()
            ...
            try:
                if CONFIG_FILE.exists():
                    dash_cfg = json.loads(CONFIG_FILE.read_text())
                    if "enabled" in dash_cfg:
                        dash_enabled = dash_cfg["enabled"]
            except Exception:
                pass

            env_enabled = os.getenv("BOT_ENABLED", "true").lower() == "true"
            current_enabled = dash_enabled and env_enabled # Combined check

            current_direction = os.getenv("BOT_DIRECTION", args.direction).upper()
            current_min_score = int(os.getenv("BOT_MIN_SCORE", args.min_score))
            current_min_score_gap = int(os.getenv("BOT_MIN_SCORE_GAP", args.min_score_gap))
            
            # Sync args so scanners and pick_candidates see the same values
            args.direction = current_direction
            args.min_score = current_min_score
            args.min_score_gap = current_min_score_gap
            ...
            try:
                config_data = {
                    ...
                    "min_score_gap": current_min_score_gap,
                    "timeframe": args.timeframe,
                    "timestamp": datetime.datetime.now().isoformat()
                }
            ...
            if not current_enabled:
                tui_log(Fore.YELLOW + f"Bot is DISABLED (Env:{env_enabled}, Dash:{dash_enabled}). Sleeping...")
                time.sleep(15)
                continue
            ...
            candidates = []
            if balance >= 10.0:
                tui_log(f"Scanning LIVE market [{current_direction}] ({args.timeframe})... [Balance: ${balance:.2f}]")
                if SHOW_PROGRESS:
                    _display_paused.set()
                
                t0 = time.time()
                # run_scanner_both now respects args.direction internally
                long_r, short_r = bot_core.run_scanner_both(cfg, args, on_result=on_scan_result, show_progress=SHOW_PROGRESS)
                ...
                if long_r:
                    top_l = max(r["score"] for r in long_r)
                    tui_log(f"  Top LONG: {top_l}")
                if short_r:
                    top_s = max(r["score"] for r in short_r)
                    tui_log(f"  Top SHORT: {top_s}")
    
                now_dt = datetime.datetime.now()
                # Directional filtering already happens inside run_scanner_both, 
                # but we double-check here for extra safety during candidate picking.
                fresh_long  = [r for r in long_r  if is_fresh(r, now_dt)] if current_direction in ["LONG", "BOTH"] else []
                fresh_short = [r for r in short_r if is_fresh(r, now_dt)] if current_direction in ["SHORT", "BOTH"] else []
    
                # Re-check balance and positions immediately after scan
                acc_updated = load_account()
                ...
                candidates = bot_core.pick_candidates(
                    ...
                    min_score=current_min_score,
                    min_score_gap=current_min_score_gap,
                    direction_filter=current_direction,
            ...
            if candidates:
                ...
                for res, direction in candidates:
                    ...
                    if load_account().get("balance", 0.0) < 10.0:
            ...
            current_balance = load_account().get("balance", 0.0)
...
def main() -> None:
    """Parses arguments and starts the trading bot."""
    parser = argparse.ArgumentParser(description="Phemex Trading Bot (Live or Paper Trading)")
    parser.add_argument("--mode",           default="sim", choices=["live", "sim"], help="Operating mode: 'live' or 'sim' (simulation/paper trading)")
    ...
    set_mode_config(args.mode)

    print(Fore.GREEN + Style.BRIGHT + f"  🚀 Phemex TRADING Bot Starting ({BOT_MODE.upper()} mode)")
    print(f"  Market   : LIVE (api.phemex.com)")
    print(f"  Account  : {'LIVE' if BOT_MODE == 'live' else 'LOCAL'} ({ACCOUNT_FILE.name})")
    ...
    bot_core.make_entity_request("Session", data={
        "session_id": bot_core.SESSION_ID,
    ...
    try:
        sim_bot_loop(args)
        # --- Session Entity End ---
        bot_core.make_entity_request("Session", method="PUT", entity_id=bot_core.SESSION_ID, data={
