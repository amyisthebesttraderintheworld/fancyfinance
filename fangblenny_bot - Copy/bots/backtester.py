...
def get_candles(symbol: str, timeframe: str = "15m", limit: int = 500) -> List[list]:
    ...
    supported_limits = (5, 10, 50, 100, 500, 1000)
    requested_limit = max(int(limit), 1)
    api_limit = next((candidate for candidate in supported_limits if candidate >= requested_limit), supported_limits[-1])
...
def backtest_symbol(
    ...
    use_bot_logic:     bool  = False, # use dynamic lev/trail/margin from live_bot.py
) -> List[Trade]:
    ...
    for i in range(window, len(ohlcv) - 1):
        ...
        if in_pos and pos is not None and i > pos.entry_idx:
            # use pos properties (which might be dynamic)
            curr_leverage = pos.leverage
            curr_margin   = pos.margin
            curr_trail    = pos.trail_pct
            ...
            if pos.direction == "LONG":
                ...
                if take_profit_pct > 0:
                    ...
                    if c_high >= tp_level:
                        ...
                        pos.pnl_usdt     = raw_ret * curr_leverage * curr_margin
                ...
                if c_close > high_water:
                    high_water = c_close
                    stop_px = high_water * (1.0 - curr_trail)
                ...
                if c_close < low_water:
                    low_water = c_close
                    stop_px = low_water * (1.0 + curr_trail)
                if c_close >= stop_px:
                    exit_px   = stop_px * (1.0 + slip_one_side + fee_one_side)
                    raw_ret   = (pos.entry_price - exit_px) / pos.entry_price
                    pos.exit_idx     = i
        ...
        # ── Dynamic Params matching live_bot.py ─────────────────────────
        if use_bot_logic:
            if is_low_liq:
                active_lev   = 10   # LOW_LIQ_LEVERAGE
                active_trail = 0.008 # LOW_LIQ_TRAIL_PCT
                active_marg  = 5.0  # LOW_LIQ_MARGIN
            else:
                # get_score_leverage(score) logic
                if entry_score < 125: active_lev = 10
                elif entry_score < 135: active_lev = 20
                elif entry_score < 145: active_lev = 30
                elif entry_score < 155: active_lev = 40
                else: active_lev = 50
        ...
        if direction_trade == "LONG":
            ...
            stop_px   = entry_px * (1.0 - active_trail)
            high_water = entry_px
        else:
            entry_px  = n_open * (1.0 - slip_one_side - fee_one_side)
            stop_px   = entry_px * (1.0 + active_trail)
        ...
        pos = Trade(
            ...
            leverage=active_lev, margin=active_marg, trail_pct=active_trail,
...
def main():
    ...
    parser.add_argument("--bot-logic",  action="store_true",
                        help="Use variable leverage/margin/trail from live_bot.py")
    ...
    # Create scan lock to pause live/sim bots
    scan_lock_path = Path(__file__).parent / ".scan.lock"
    scan_lock_path.write_text(str(os.getpid()))
