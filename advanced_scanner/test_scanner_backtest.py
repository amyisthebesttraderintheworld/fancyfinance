...
def test_score_to_leverage():
    assert score_to_leverage(100) >= 19
    assert score_to_leverage(50) >= 5
    assert score_to_leverage(0) == 1
...
def test_score_rows_bullish():
    rows = create_synthetic_rows(100, lambda i: 100 - i if i < 90 else 10)
    rows[-1] = [99*3600, 0, 0, 9.5, 11, 9, 10.5, 5000]
    assert score_rows(rows, -0.001) > 0
...
def test_order_and_trade():
    ...
    t2 = Trade(o, 50000, 11, 1060)
    t2.close(12, 1120, 40000, force_liq=True)
    assert t2.liquidated
    assert t2.pnl_usd == -100

# --- 4. Backtest Portfolio ---

def test_backtest_simple(monkeypatch):
    import scoring as scanner_backtest
    monkeypatch.setattr(scanner_backtest, "score_rows", lambda rs, f: 50 if len(rs) == 61 else 0)
...
def test_plumbing(monkeypatch):
    ...
    import scoring as scanner_backtest
    monkeypatch.setattr(scanner_backtest.SESSION, "get", lambda *a, **kw: mock_resp)
...
def test_run_scan(monkeypatch):
    import scoring as scanner_backtest
    monkeypatch.setattr(scanner_backtest, "fetch", lambda s, f, l: (s, create_synthetic_rows(100), 0))
    ranked = run_scan(["BTC"], {})
    assert len(ranked) == 1

# --- 7. Robustness and Optimization ---

def test_robustness_and_opt(monkeypatch):
    import scoring as scanner_backtest
    monkeypatch.setattr(scanner_backtest, "backtest_portfolio", lambda *a, **kw: ([], [], 100, []))
    monkeypatch.setattr(scanner_backtest, "report_to_deepseek", lambda r: None)
    rows = create_synthetic_rows(100)
    best = optimize_parameters({"BTC": rows}, {}, {}, 100, 0.01)
    assert len(best) == 2
    monkeypatch.setattr(scanner_backtest, "fetch", lambda s, f, l: (s, rows, 0))
